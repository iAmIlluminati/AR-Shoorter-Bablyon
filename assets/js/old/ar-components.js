...
AFRAME.registerComponent("attack-effect", {
	init: function () {
		attackEffect = this.el
		attackEffect.setAttribute('visible', false);
	},
});
...
// AFRAME.registerComponent("init-camera", {
// 	init: function () {
// 		aCamera = this.el
// 		console.log(aCamera)
// 	},
...
AFRAME.registerComponent("init-player-body", {
	init: function () {
		// aPlayer = this.el
			// aCamera.setAttribute('shader', 'uniforms', {hue: 10.2});
			aPlayer.addEventListener('collide', function (e) {
				// console.log('Player has collided with '+e.detail.body.el.id);
				if (e.detail.body.el.id.includes('attack-ball'))   
				{if(attackEffect&&healthBarMain){
					attackEffect.setAttribute('visible', true);
					AttackOnHealth()
					setTimeout(function(){
						attackEffect.setAttribute('visible', false);	
					}, 700);
				}}else if(e.detail.body.el.id.includes('sprite')){
					removeSprite(e.detail.body.el.id)
				}
			});
	},
});
